...
\section{Experimentálne vyhodnotenie lokálneho TF.js modelu}

Lokálny TF.js model bol vyhodnotený ako prvý orientačný variant inferencie priamo v prehliadači. Cieľom tejto časti je zistiť, do akej miery je predtrénovaný model použiteľný ako základ pre režim orientovaný na súkromie používateľa. Vyhodnotenie sa sústreďuje na základné metriky viacštítkovej klasifikácie, správanie jednotlivých kategórií toxicity a obmedzenia vyplývajúce z použitia jedného globálneho rozhodovacieho prahu.

\subsection{Nastavenie experimentu}
Pre lokálny režim bol otestovaný predtrénovaný model \emph{@tensorflow-models/toxicity} spúšťaný priamo v prehliadači. Cieľom testu bolo získať orientačný obraz o kvalite detekcie v režime \emph{privacy-by-default}, teda bez odosielania textu mimo klienta. Keďže lokálne nasadenie je limitované výkonom a kompatibilitou backendov, meranie bolo vykonané s backendom \emph{CPU} a so zjednodušeným rozhodovaním pomocou jedného globálneho prahu $\tau=0{,}85$.

Vyhodnotenie prebehlo na podmnožine \emph{val.csv}, z ktorej bolo použitých 5\,000 riadkov. Reportované sú štandardné multi-label metriky, konkrétne macro-F1, micro-F1, Hammingova strata a F1 pre jednotlivé štítky.
...
\begin{table*}[!htbp]
  ...
  \begin{tabularx}{\textwidth}{|P{3.6cm}|Y|Y|Y|Y|Y|}
    \hline
    \textbf{Konfigurácia} &
    \textbf{Backend} &
    \textbf{TF.js ver.} &
    ...
    \multicolumn{6}{|P{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth\relax}|}{%
      \textbf{Metriky:} macro-F1 = 0{,}283;\quad micro-F1 = 0{,}568;\quad Hamming loss = 0{,}0236.
    } \\
    \hline
  \end{tabularx}
  \caption[Vyhodnotenie lokálneho TF.js modelu]{Súhrn vyhodnotenia lokálneho TF.js modelu (\emph{@tensorflow-models/toxicity}) na podmnožine validačných dát.}
\end{table*}
\FloatBarrier

\subsection{Výsledky a interpretácia}
Výsledky naznačujú, že lokálny model dosahuje použiteľnú výkonnosť najmä pre častejšie triedy, napríklad \emph{toxic} a \emph{insult}, avšak pri menšinových kategóriách, ako sú \emph{threat}, \emph{severe\_toxic} a \emph{identity\_hate}, pri zvolenom globálnom prahu nedosahuje pozitívny F1. Tento jav je v súlade s očakávaniami z analytickej časti: menšinové triedy sú náročné na citlivé prahovanie a lokálny model je primárne vhodný ako \emph{privacy-friendly baseline} alebo ako režim s nižšími nárokmi na presnosť.

Z praktického pohľadu to podporuje architektúru s dvomi režimami inferencie, kde lokálny režim poskytuje okamžitú ochranu súkromia a vzdialený režim môže slúžiť ako presnejší opt-in doplnok.
...
\begin{table*}[!htbp]
  ...
  \begin{tabularx}{\textwidth}{|p{2.8cm}|*{6}{>{\centering\arraybackslash}X|}}
    \hline
    \textbf{Štítok} &
    \textbf{Toxic} &
    \textbf{Severe toxic} &
    ...
    \hline
  \end{tabularx}
  \caption[Per-label F1 pre lokálny model]{Per-label F1 skóre lokálneho TF.js modelu pri globálnom prahu $\tau = 0{,}85$.}
\end{table*}
\FloatBarrier
...
\begin{table*}[!htbp]
  ...
  \begin{tabularx}{\textwidth}{|p{4.4cm}|Y|Y|Y|Y|Y|}
    \hline
    \textbf{Konfigurácia} &
    \textbf{Model} &
    \textbf{TF.js ver.} &
